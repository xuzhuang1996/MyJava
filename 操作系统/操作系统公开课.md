...
## 内存管理
1. [内存层次](https://wudaijun.com/2019/04/cpu-cache-and-memory-model/)。磁盘等属于外存

   <img src="https://github.com/xuzhuang1996/MyJava/blob/master/img/操作系统/内存层次.PNG" width=70% height=70% />
   
     >1. MMU内存管理单元，处理CPU存储访问请求的硬件
     >2. 从CPU寄存器到硬盘，越靠近CPU的存储器越小越快，离CPU越远的存储器越大但越慢
     >3. 缓存的出现主要是为了解决CPU运算速度与内存读写速度不匹配的矛盾。CPU 需要数据就问缓存要，缓存没有就从主存中读取，并保留一份在缓存中。下次读取就从缓存中读取，加快速度。
     >4. 多线程下缓存出现不一致，需要一种协议，来保证不同的线程在读写主存数据时遵守某种规则以保证不会出现数据不一致的问题。用的比较多的是 MESI 协议。除此以外，还需要内存屏障。
2. 地址空间
   1. 地址总线：物理地址总线条数。32位即32条地址线，内存从0~4G-1。
