...
## spring
3. Autowired、Resource两者区别：一旦涉及到泛型。如T为beanA，Autowired将根据IUserService<beanA>注入，而Resource将根据IUserService<T>进行注入。如果此时有2个以上的IUserService<T>类型，虽然T不一样，但是依然会产生冲突，报错。
