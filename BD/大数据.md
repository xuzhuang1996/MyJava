...
2. spark
   1. MapReduce需要大量的磁盘I/O,MapReduce会将中间处理的大量的数据存在HDFS上，而Spark因为是内存式的，就不需要大量的磁盘I/O，这一块就会非常快。因为它通过弹性分布式数据集（RDD）将大部分输入数据缓存在内存中，RDD管理数据的分布式处理和数据的转换。
   2. RDD是不能被更改的，它只能经过一系列的“转换”操作产生新的RDD
      - 窄依赖：一个父RDD的分区对应于一个子RDD的分区，或多个父RDD的分区对应于一个子RDD的分区，即多对一
      - 宽依赖：存在一个父RDD的一个分区对应一个子RDD的多个分区，即一对多。通常伴随Shuffle操作。
   2. scala更少的代码量
## 安装
